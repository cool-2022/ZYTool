...
async def generate_stream(message: str):
    ...
    yield "data: [DONE]\n\n"
async def generate_stream_simple(message: str):
    """流式生成回复，不使用 Function Call（兼容不支持工具调用的模型）"""
    for char in deepseek_client.chat_stream(message):
        yield f"data: {char}\n\n"
        await asyncio.sleep(0.01)
    yield "data: [DONE]\n\n"

async def generate_stream_no_tools(message: str):
    """流式生成回复，不支持 Function Call"""
    try:
...
def generate_sync_simple(message: str) -> str:
    """同步生成回复，不使用 Function Call"""
    result = deepseek_client.chat(message)
