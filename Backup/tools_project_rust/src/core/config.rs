...
/// 应用全局配置结构体，字段优先从环境变量读取，缺失时使用默认值。
...
fn default_cors_origins() -> Vec<String> {
    vec![
        ...
        // 局域网访问（本机 IP，供同网段其他设备访问前端）
        "http://192.168.77.154:5050".to_string(),
        "http://192.168.77.154:5173".to_string(),
        "http://192.168.188.2:5050".to_string(),
        "http://192.168.188.2:5173".to_string(),
    ]
}
...
// ===== Kimi（Moonshot AI）默认值 =====
fn default_kimi_base_url() -> String { "https://api.moonshot.cn/v1".to_string() }
fn default_kimi_model() -> String { "kimi-k2-0905-preview".to_string() }

/// 全局唯一配置实例，首次访问时从 `.env` 文件和环境变量加载，失败则回退到默认配置。
pub static SETTINGS: Lazy<Settings> = Lazy::new(|| {
    // 尝试加载项目根目录的 .env 文件，不存在或加载失败时忽略错误
    dotenvy::dotenv().ok();
    // 使用 envy 将环境变量反序列化为 Settings，失败时使用硬编码默认配置
